...
def fixed_pillar_over_policy(obs):
    curr_state = obs[:2]
    goal_state  = obs[2:4]
    ...
    # If goal is left of pillar or below pillar, proceed as normal
    if goal_x < pillar_x or goal_y < (pillar_y - 1.0):
...
def fixed_pillar_under_policy(obs):
    curr_state = obs[:2]
    goal_state  = obs[2:4]
    pillar_state = obs[4:] # TODO: assuming pillar is 1x1
    curr_x, curr_y = curr_state
